// mexFunction for PR-4Pi localization based on linear interpolation with 11 fitting parameters:x,y,z,I1,I2,I3,I4,bg1,bg2,bg3,bg4
// call mex function in Matlab:
// [P,CG,crlb,err,PSF] = cuda4pi_loc_linear(data,cor,PSFsi,SampleSpacingXY,SampleSpacingZ,StartX,StartY,StartZ,iterateNum,channelNum,lambda0,xIn,gainR);
// 
 ...
 */
	...
	IT = (float*)malloc(4 * channelNum*sizeof(float));
	bgT = (float*)malloc(4 * channelNum*sizeof(float));
...
void CalNextParam(Model_Localization &obj, float *x0_next, float *x0, float *Converg, int channelNum)
{
	...
	I = (float*)mxMalloc(4*channelNum*sizeof(float));
	bg = (float*)mxMalloc(4*channelNum*sizeof(float));
...
void PopulArray(float *x, float *y, float *z, float *I, float *bg, float *err, int channelNum, float *x0)
{
	...
	for (j = 0; j < channelNum; j++)
	{
		...
		for (s = 0; s < 4; s++)
		{
			...
			I[s*channelNum + j] = x0[j*NPL + s + 3];
			bg[s*channelNum + j] = x0[j*NPL + s + 7];
