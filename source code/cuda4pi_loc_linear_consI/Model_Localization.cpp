...
void Model_Localization::setup(){
	...
	cudaMalloc(&d_SampledPSF, 4*SampledPSF_XYPixels*SampledPSF_XYPixels*SampledPSF_ZPixels*sizeof(float));
	cudaMalloc(&d_X, Nfit*sizeof(float));
	cudaMalloc(&d_Y, Nfit*sizeof(float));
	cudaMalloc(&d_Z, Nfit*sizeof(float));
	cudaMalloc(&d_I, Nfit*sizeof(float));
	cudaMalloc(&d_Bg, Nfit*sizeof(float));
	...
	cudaMemcpy(d_SampledPSF, SampledPSF, 4 * SampledPSF_XYPixels*SampledPSF_XYPixels*SampledPSF_ZPixels*sizeof(float), cudaMemcpyHostToDevice);
...
void Model_Localization::sendParams(float *X, float *Y, float *Z, float *I, float *Bg, float *ParamIn, float *Error){
	...
	cudaMemcpy(d_I, I, Nfit*sizeof(float), cudaMemcpyHostToDevice);
	cudaMemcpy(d_Bg, Bg, Nfit*sizeof(float), cudaMemcpyHostToDevice);
...
void Model_Localization::mLocalization(){
	...
	for (int s = 0; s < 4; s++)
	{
		kernel_calcPSFPixel_wrapper(dimGrid, dimBlock, &d_SampledPSF[s*Samplepsf_Pixels], SampledPSF_XYPixels, SampledPSF_XYPixels, 
		...
		kernel_getdev_wrapper(dimGrid2, dimBlock2, &d_Data[s*Nfit*PSFSize], &d_GainR[s*Nfit*PSFSize], d_PSFs, d_dPSFx, d_dPSFy, d_dPSFz, 
			d_I, d_Bg, Nfit, PSFSize, &d_FirstDev[s*NP*Nfit], &d_SecondDev[s*NP*Nfit]);
...
void Model_Localization::calPSF(){
	...
	for (int s = 0; s < 4; s++)
	{
		...
		kernel_calcPSFI_wrapper(dimGrid2, dimBlock2, d_PSFs, d_PSFI, d_I, d_Bg, Nfit, PSFSize);
...
void Model_Localization::calCRLB(){
	...
	for (int s = 0; s < 4; s++)
	{
		...
		kernel_calFisherM_wrapper(dimGrid2, dimBlock2, d_PSFs, d_dPSFx, d_dPSFy, d_dPSFz, d_I, d_Bg, &d_GainR[s*Nfit*PSFSize], &d_ParamF[s*NPL*NPL*Nfit],
			s, Nfit, PSFSize);
