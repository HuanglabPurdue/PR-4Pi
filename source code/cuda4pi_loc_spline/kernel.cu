...
__global__ void kernel_calcPSFPixel(float *SamplePSF, float *SamplePSFx, float *SamplePSFy, float *SamplePSFz, float *SamplePSFxy, float *SamplePSFxz, float *SamplePSFyz, float *SamplePSFxyz, 
	int SizeX, int SizeY, float *PSFs, float *dPSFx, float *dPSFy, float *dPSFz, float * X, float *Y, float *Z, float *transx, float *transy,
...

{
	...
	if ((threadIdx.x == 0) && (threadIdx.y == 0)){
		theta[0] = X[blockIdx.y] * SampleSpacingXY * N_int + transx[blockIdx.y] * SampleSpacingXY * N_int;
		theta[1] = Y[blockIdx.y] * SampleSpacingXY * N_int + transy[blockIdx.y] * SampleSpacingXY * N_int;
	...
	if (threadIdx.x == 0)
	{
		...
		//dPSFx2_Row_Sum[threadIdx.y] = 0;
		//dPSFy2_Row_Sum[threadIdx.y] = 0;
		//dPSFz2_Row_Sum[threadIdx.y] = 0;
	}
	for (int ii = 0; ii < N_int; ii++) //go right in row
	{
		...
		//dPSFz2_Row_Samples[N_int*PixelNumber + idX] = dF[1];
		...
		dPSFy_Row_Samples[N_int*PixelNumber + idX] = evalspline(a, b, c, d, theta[2]);
			...
			// here we ignore the second derivative
		//gencoeff(dF2d_x2[0], dF2d_x2[1], dF2d_zx2[0], dF2d_zx2[1], SampleSpacingZ, X1z[0], X1z[1], &a, &b, &c, &d);
		//dPSFx2_Row_Samples[N_int*PixelNumber + idX] = evalspline(a, b, c, d, theta[2]);

		//gencoeff(dF2d_y2[0], dF2d_y2[1], dF2d_zy2[0], dF2d_zy2[1], SampleSpacingZ, X1z[0], X1z[1], &a, &b, &c, &d);
		//dPSFy2_Row_Samples[N_int*PixelNumber + idX] = evalspline(a, b, c, d, theta[2]);
		...
		for (int jj = 0; jj < N_int; jj++)
		{
			...
			//dPSFx2_Row_Sum[threadIdx.y] += dPSFx2_Row_Samples[N_int*PixelNumber + jj];
			//dPSFy2_Row_Sum[threadIdx.y] += dPSFy2_Row_Samples[N_int*PixelNumber + jj];
			//dPSFz2_Row_Sum[threadIdx.y] += dPSFz2_Row_Samples[N_int*PixelNumber + jj];
	...
	if (threadIdx.x == 0)
	{
		...
		//dPSFx2[PSFSizeOut*PSFSizeOut*blockIdx.y + PSFSizeOut*blockIdx.x + threadIdx.y] = dPSFx2_Row_Sum[threadIdx.y];
		//dPSFy2[PSFSizeOut*PSFSizeOut*blockIdx.y + PSFSizeOut*blockIdx.x + threadIdx.y] = dPSFy2_Row_Sum[threadIdx.y];
		//dPSFz2[PSFSizeOut*PSFSizeOut*blockIdx.y + PSFSizeOut*blockIdx.x + threadIdx.y] = dPSFz2_Row_Sum[threadIdx.y];
