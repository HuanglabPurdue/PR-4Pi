...
//*******************************************************************************************
//theta is: {x,y,N,bg}
__global__ void kernel_MLEFit_noshared(const float *d_data, float *d_Coords, const float *d_GainRatio, 
	...
	float *s_Coords = d_Coords+(2*bx*BlockSize+2*tx);
...
//*******************************************************************************************
__global__ void kernel_MLEFit_sigma(const float *d_data, float *d_Coords, const float *d_GainRatio,
	    const float PSFSigma, const int sz, const int Mapsz, const int iterations, 
        float *d_Parameters, float *d_CRLBs, float *d_LogLikelihood,const int Nfits){
	/*! 
	 * \brief basic MLE fitting kernel.  No additional parameters are computed.
...
//*******************************************************************************************
__global__ void kernel_MLEFit_z(const float *d_data, float *d_Coords, const float *d_GainRatio, float *d_x0,
    ...
    const float *s_data = d_data+(sz*sz*bx*BlockSize+sz*sz*tx);
	float *s_Coords = d_Coords+(2*bx*BlockSize+2*tx);
	float *z_initial = d_x0+(bx*BlockSize+tx);
    //initial values
    kernel_CenterofMass2D(sz, s_data, &theta[0], &theta[1]);
...
//*******************************************************************************************
__global__ void kernel_MLEFit_sigmaxy(const float *d_data, float *d_Coords, const float *d_GainRatio, 
	    const float PSFSigma, const int sz, const int Mapsz, const int iterations, 
        float *d_Parameters, float *d_CRLBs, float *d_LogLikelihood,const int Nfits){
	/*! 
	 * \brief basic MLE fitting kernel.  No additional parameters are computed.
