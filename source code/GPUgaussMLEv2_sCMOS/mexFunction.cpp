...
extern void kernel_MLEFit_noshared_wrapper(dim3 dimGrid, dim3 dimBlock, const float *d_data, float *d_Coords, const float *d_GainRatio, const float PSFSigma, const int sz, const int Mapsz, const int iterations, 
...
extern void kernel_MLEFit_sigma_wrapper(dim3 dimGrid, dim3 dimBlock, const float *d_data, float *d_Coords, const float *d_GainRatio, const float PSFSigma, const int sz, const int Mapsz, const int iterations, 
	float *d_Parameters, float *d_CRLBs, float *d_LogLikelihood,const int Nfits);

extern void kernel_MLEFit_z_wrapper(dim3 dimGrid, dim3 dimBlock, const float *d_data, float *d_Coords, const float *d_GainRatio, float *d_x0, const float PSFSigma_x, const float Ax, const float Ay, const float Bx, 
	const float By, const float gamma, const float d, const float PSFSigma_y, const int sz, const int Mapsz, const int iterations, 
	float *d_Parameters, float *d_CRLBs, float *d_LogLikelihood,const int Nfits);

extern void kernel_MLEFit_sigmaxy_wrapper(dim3 dimGrid, dim3 dimBlock, const float *d_data, float *d_Coords, const float *d_GainRatio, const float PSFSigma, const int sz, const int Mapsz, const int iterations, 
	float *d_Parameters, float *d_CRLBs, float *d_LogLikelihood,const int Nfits); 
...
void mexFunction(int nlhs, mxArray *plhs[],	int	nrhs, const	mxArray	*prhs[]) {
	...
	*/
	...
	int blockx=0;
	int threadx=0;
	const mwSize *datasize=0, *Coordsize=0, *Mapsize=0,*x0size=0;
	float PSFSigma=0, Ax=0, Ay=0, Bx=0, By=0, gamma=0, d=0, PSFSigma_y=0;
	int iterations=0;
	float *data=0, *d_data=0;
	float *Coords=0, *d_Coords=0;
	float *GainRatio=0, *d_GainRatio=0;
	float *x0,*d_x0;
	float *d_Parameters=0,*d_CRLBs=0,*d_LogLikelihood=0;
	size_t Ndim, Nfitraw, fittype, sz, Mapsz;
	...
	Coordsize=mxGetDimensions(prhs[1]);
	Mapsize=mxGetDimensions(prhs[2]);
	...
	if (Coordsize[1]!=Nfitraw)
		mexErrMsgIdAndTxt("GPUgaussMLEv2:CoordSize","Coordinates number must be equal to the subregion number.\n");
	if (Coordsize[0]!=2)
		mexErrMsgIdAndTxt("GPUgaussMLEv2:CoordSize","Coords Must include both x,y coordinates.\n");
	if (Mapsize[0]!=Mapsize[1])
		mexErrMsgIdAndTxt("GPUgaussMLEv2:MapSize","x,y dimension of Gain ratio map must be the same.\n");
	...
	Coords=(float *) mxGetData(prhs[1]);
	GainRatio=(float *) mxGetData(prhs[2]);
	
	//for(int k=0;k<Nfitraw;k++)mexPrintf("%d  %d\n",(int)Coords[2*k],(int)Coords[2*k+1]);
	PSFSigma=(float)mxGetScalar(prhs[3]); //matlab-dip_image convention
	iterations=(int) mxGetScalar(prhs[4]);
	fittype = (int) mxGetScalar(prhs[5]);
	...
	if(nrhs>13)
		{
			x0=(float *) mxGetData(prhs[13]);
			x0size=mxGetDimensions(prhs[13]);			
			if (x0size[0]!=Nfitraw)
		    mexErrMsgIdAndTxt("GPUgaussMLEv2:x0size","initial parameter number must be equal to the subregion number.\n");
	}
	...
	cudasafe(cudaMalloc((void**)&d_Coords, 2*Nfitraw*sizeof(float)), "Failed cudaMalloc on d_Coords.",__LINE__);
	cudasafe(cudaMemset(d_Coords,0,2*Nfitraw*sizeof(float)),"Failed cudaMemset on d_Coords.",__LINE__);
	cudasafe(cudaMemcpy(d_Coords,Coords,2*Nfitraw*sizeof(float), cudaMemcpyHostToDevice), "Failed cudaMemcpy on d_Coords.", __LINE__);

	cudasafe(cudaMalloc((void**)&d_GainRatio, Mapsz*Mapsz*sizeof(float)),"Failed cudaMalloc on d_Coords.",__LINE__);
	cudasafe(cudaMemset(d_GainRatio, 0, Mapsz*Mapsz*sizeof(float)), "Failed cudaMemset on d_Coords.",__LINE__);
	cudasafe(cudaMemcpy(d_GainRatio, GainRatio, Mapsz*Mapsz*sizeof(float), cudaMemcpyHostToDevice), "Failed cudaMemcpy on d_GainRatio.",__LINE__);
	...
	case 3: // (x,y,bg,I,z)
		...
		cudasafe(cudaMalloc((void**)&d_x0, Nfitraw*sizeof(float)),"Failed cudaMalloc on d_x0.",__LINE__);
		cudasafe(cudaMemset(d_x0, 0, Nfitraw*sizeof(float)), "Failed cudaMemset on d_x0.",__LINE__);
		cudasafe(cudaMemcpy(d_x0, x0, Nfitraw*sizeof(float), cudaMemcpyHostToDevice), "Failed cudaMemcpy on d_x0.",__LINE__);
	...
	switch(fittype) {
	case 1: //fit x,y,bg,I
		kernel_MLEFit_noshared_wrapper(dimGrid, dimBlock, d_data, d_Coords, d_GainRatio, PSFSigma, (int) sz, (int) Mapsz, iterations, d_Parameters, d_CRLBs, d_LogLikelihood, (const int) Nfitraw);
	...
	case 2: //fit x,y,bg,I,sigma
		kernel_MLEFit_sigma_wrapper(dimGrid, dimBlock, d_data, d_Coords, d_GainRatio, PSFSigma, (int) sz, (int) Mapsz, iterations, d_Parameters, d_CRLBs, d_LogLikelihood, (const int) Nfitraw);
		break;

	case 3: //fit x,y,bg,I,z
		kernel_MLEFit_z_wrapper(dimGrid, dimBlock, d_data, d_Coords, d_GainRatio, d_x0, PSFSigma, Ax, Ay, Bx, By, gamma, d, PSFSigma_y, (int) sz, (int) Mapsz, iterations, d_Parameters, d_CRLBs, d_LogLikelihood, (const int) Nfitraw);
		//mexPrintf("A:%f B:%f gamma:%f d:%f \n",Ax,Bx,gamma,d);
		break;

	case 4: //fit x,y,bg,I,sigmax,sigmay
		kernel_MLEFit_sigmaxy_wrapper(dimGrid, dimBlock, d_data, d_Coords, d_GainRatio,PSFSigma, (int) sz, (int) Mapsz, iterations, d_Parameters, d_CRLBs, d_LogLikelihood, (const int) Nfitraw);
	...
	cudasafe(cudaFree(d_Coords),"cudaFree failed on d_Coords.",__LINE__);
	cudasafe(cudaFree(d_GainRatio),"cudaFree failed on d_GainRatio.",__LINE__);
	...
	if (fittype==3){cudasafe(cudaFree(d_x0),"cudaFree failed on d_x0.",__LINE__);}
