classdef CalCRLB_4pi < handle
    ...
    %
    % CalCRLB_4pi class for calculating CRLB of 4Pi PSF models with 11 fitting parameters: x, y, z, I1, I2, I3, I4, bg1, bg2, bg3, bg4
    %   create object: obj = CalCRLB_4pi(PRstruct)
    %
    % CalCRLB_4pi Methods:
    ...
    methods
        function obj = CalCRLB_4pi(PRstruct)
...
%                psfI = obj.PSFobj.PSFs;
            ...
            for s = 0:N-1
                for nn = 1:4
                    t = s+1;
                    psf = psfL(:,:,:,nn);
                    vec_bg = zeros(obj.Boxsize,obj.Boxsize,4);
                    vec_bg(:,:,nn) = 1;
                    vec_I = zeros(obj.Boxsize,obj.Boxsize,4);
                    vec_I(:,:,nn) = psf(:,:,s*pN+1);
                    %x
            ...
            obj.Photon_STD = sqrt(obj.CRLB(:,4:7));
            obj.Bg_STD = sqrt(obj.CRLB(:,8:11));
